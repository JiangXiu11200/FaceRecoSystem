...
function Group() {
    ...
    const groupDetailDialogFooter = (
        <React.Fragment>
            <div className="d-flex flex-row justify-content-end">
                <div className="me-2">
                    <Button label="Cancel" icon="pi pi-times" className="p-button-text cancel-btn" onClick={hideGroupDetailDialog} />
                </div>
                <div>
                    {!edit_group && !create_group ? null : create_group ? (
                        <Button label="Create" icon="pi pi-check" className="p-button-text func-btn" onClick={hideGroupDetailDialog} />
                    ) : (
                        <Button label="Save" icon="pi pi-check" className="p-button-text func-btn" onClick={hideGroupDetailDialog} />
                    )}
                </div>
            </div>
        </React.Fragment>
    ...
    const deleteGroupDialogFooter = (
        <React.Fragment>
            <div className="d-flex flex-row justify-content-end">
                ...
                <div>
                    <Button label="Delete" icon="pi pi-times" className="p-button-text delete-btn" onClick={hideGroupDetailDialog} />
                </div>
            </div>
        </React.Fragment>
    )
    ...
    return (
        <div className="d-flex flex-column">
            <Toolbar className="toolbar-layout" left={leftContents} right={rightContents} />
            <Table data={products} columns={columns} actnioEvent={setActionEvent} editFlag={true} deleteFlag={true} viewsFlag={true} />
            <Dialog
                visible={set_group_dialog}
                className=""
                header={create_group ? "Create Group" : edit_group ? "Edit Group" : "Group Details"}
            ...
            <Dialog visible={set_delete_dialog} className="" header="Delete Group" footer={deleteGroupDialogFooter} onHide={hideDeleteGroupDialog}>
                <div className="d-flex flex-row align-items-center">
                    <div>
